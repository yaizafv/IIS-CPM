...
import uo.cpm.p12.model.Articulo;
import uo.cpm.p12.service.McDonalds;
...
public class VentanaPrincipal extends JFrame {
	...
	private ProcesaBotonFiltro pB;
	private ProcesaSupr pS;
	...
	private JScrollPane getPnPrincipal() {
		if (pnPrincipal == null) {
			...
			pnPrincipal.setRowHeaderView(getPnFiltro());
	...
	private JTabbedPane getPnPedido() {
		if (pnPedido == null) {
			...
			pnPedido.setMnemonicAt(0, 67);
			pnPedido.addTab("Bebida", null, getScBebida(), null);
			pnPedido.setMnemonicAt(1, 66);
		}
		return pnPedido;
	}
	...
	private JList<Articulo> getListComida() {
		if (listComida == null) {
			...
			listComida.setToolTipText("Pulse Supr para eliminar un articulo");
		}
		return listComida;
	}
	...
	private JList<Articulo> getListBebida() {
		if (listBebida == null) {
			...
			listBebida.setToolTipText("Pulse Supr para eliminar un articulo");
		}
		return listBebida;
	}
	...
	private JPanel getPnFiltro() {
		if (pnFiltro == null) {
			...
			pnFiltro.add(getBtHamburguesa());
			pnFiltro.add(getBtBebida());
			pnFiltro.add(getBtComplemento());
	...
	private JButton getBtTodos() {
		if (btTodos == null) {
			...
			btTodos.addActionListener(pB);
			btTodos.setMnemonic('t');
			btTodos.setMargin(new Insets(0, 3, 0, 3));
	...
	private JButton getBtPostre() {
		if (btPostre == null) {
			btPostre = new JButton("Postre");
			btPostre.addActionListener(pB);
			btPostre.setMnemonic('p');
			btPostre.setMargin(new Insets(0, 3, 0, 3));
	...
	private JButton getBtHamburguesa() {
		if (btHamburguesa == null) {
			...
			btHamburguesa.addActionListener(pB);
			btHamburguesa.setMnemonic('h');
			btHamburguesa.setMargin(new Insets(0, 3, 0, 3));
	...
	private JButton getBtBebida() {
		if (btBebida == null) {
			...
			btBebida.addActionListener(pB);
			btBebida.setMnemonic('d');
			btBebida.setMargin(new Insets(0, 3, 0, 3));
			btBebida.setVerticalTextPosition(SwingConstants.BOTTOM);
			btBebida.setHorizontalTextPosition(SwingConstants.CENTER);
			btBebida.setIcon(new ImageIcon(VentanaPrincipal.class.getResource("/img/Bebidas.png")));
		}
		return btBebida;
	}
	...
	private JButton getBtComplemento() {
		if (btComplemento == null) {
			btComplemento = new JButton("Complemento");
			btComplemento.addActionListener(pB);
			btComplemento.setMnemonic('o');
			btComplemento.setMargin(new Insets(0, 3, 0, 3));
			btComplemento.setVerticalTextPosition(SwingConstants.BOTTOM);
			btComplemento.setHorizontalTextPosition(SwingConstants.CENTER);
			btComplemento.setIcon(new ImageIcon(VentanaPrincipal.class.getResource("/img/Complementos.png")));
		}
		return btComplemento;
	}

	public void mostrarArticulos(String tipo) {
		mcDonalds.setTipoFiltro(tipo);
		deshabilitaBotones();
	}

	private void deshabilitaBotones() {
		for (int i = 0; i < getPnArticulos().getComponentCount(); i++) {
			JButton bt = (JButton) getPnArticulos().getComponent(i);
			int posicion = Integer.parseInt(bt.getActionCommand()); // posicion del articulo
			bt.setEnabled(mcDonalds.isArticuloFiltrado(posicion));
		}
	}
	...
	class ProcesaBotonFiltro implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			JButton bt = (JButton) e.getSource();
			mostrarArticulos(bt.getText());
		}
	}

	public void eliminarArticulo(KeyEvent e) {
		// Obtenemos la lista (JList) sobre la que hemos presionado Supr
		...
		// Comprobamos que se haya pulsado supr con algun elemento seleccionado
		if (a != null) {
	...
	private void habilitarBotonesPnPrincipal(boolean estado) {
		getBtSig1().setEnabled(estado);
		getBtAnular1().setEnabled(estado);
	}

	private void mostrarPrecioPedido() {
