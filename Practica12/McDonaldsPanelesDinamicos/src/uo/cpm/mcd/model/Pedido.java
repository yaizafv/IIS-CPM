...
public class Pedido {
	...
	public List<Articulo> getArticulosEnPedido(String tipo){
		...
		List<Articulo> listaPedidoPorTipo = new ArrayList<Articulo>();
		if (tipo.equals("Bebida"))
			for (Articulo a : listaPedido){
				if (a.getTipo().equals(tipo))   // cambiar la condición dentro
					listaPedidoPorTipo.add(a); 
			}
		else
			for (Articulo b : listaPedido){
				if (!b.getTipo().equals("Bebida"))
				listaPedidoPorTipo.add(b);
			}
		return listaPedidoPorTipo;
