...
import uo.cpm.p10.player.MusicPlayer;
import uo.cpm.p10.player.Song;
...
public class VentanaPrincipal extends JFrame {
	...
	private JMenuItem getMniContents() {
		if (mniContents == null) {
			...
			mniContents.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_F1, 0));
			mniContents.setMnemonic('c');
			mniContents.setEnabled(true);
		}
		return mniContents;
	}
	...
	private void cargaAyuda(){
		   ...
		   hb.enableHelpOnButton(getMniContents(), "intro", hs);
		   hb.enableHelp(getListLibrary(), "add", hs);
		   hb.enableHelp(getBtDel(), "delete", hs);
		   hb.enableHelp(getListPlay(), "play", hs);
		   hb.enableHelp(getBtPlay(), "play", hs);
